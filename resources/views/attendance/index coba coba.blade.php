                                    ...
                                    <div class="card-body">
                                        <span style="font-size: 16px">
                                            <div class="d-flex justify-content-between">
                                                ...
                                                @if (Auth::guard('staff')->check() == true)
                                                <div class="dropdown">
                                                    <button class="btn btn-sm" type="button" id="dropdownMenuButton{{ $key }}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="padding: 0 5px; background: none;">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton{{ $key }}">
                                                        ...
                                                        <a href="javascript:void(0)" class="dropdown-item" data-toggle="modal"
                                                            ...
                                                            onclick="updateModalReg({{ $key }})">
                                                            <i class="fas fa-pencil-alt me-2 text-success"></i> Edit
                                                        </a>

                                                        <button type="button" class="dropdown-item" data-toggle="modal" data-target="#exampleModal"
                                                            data-priceid="{{ $item->program }}" data-idteacher="{{ $item->teacher_name }}"
                                                            data-day1="{{ $item->day_one }}" data-day2="{{ $item->day_two }}"
                                                            data-time="{{ $item->course_time }}" data-day1-fix="{{ $item->day1 }}"
                                                            data-day2-fix="{{ $item->day2 }}" data-idteacher-fix="{{ $item->id_teacher }}"
                                                            data-priceid-fix="{{ $item->priceid }}">
                                                            <i class="fas fa-exchange-alt me-2 text-warning"></i> Transfer
                                                        </button>

                                                        <button type="button" class="dropdown-item" data-toggle="modal" data-target="#editStarModal"
                                                            data-priceid="{{ $item->program }}" data-idteacher="{{ $item->teacher_name }}"
                                                            data-day1="{{ $item->day_one }}" data-day2="{{ $item->day_two }}"
                                                            data-time="{{ $item->course_time }}" data-day1-fix="{{ $item->day1 }}"
                                                            data-day2-fix="{{ $item->day2 }}" data-idteacher-fix="{{ $item->id_teacher }}"
                                                            data-priceid-fix="{{ $item->priceid }}">
                                                            <i class="fas fa-star me-2 text-info"></i> Star
                                                        </button>
                                                        ...
                                                        <button type="button" class="dropdown-item" data-toggle="modal" data-target="#exampleModal"
                                                            ...
                                                            data-day2-fix="{{ $item->day2 }}" data-idteacher-fix="{{ $item->id_teacher }}"
                                                            data-priceid-fix="{{ $item->priceid }}">
                                                            <i class="fas fa-user-plus me-2 text-primary"></i> Assist Class
                                                        </button>
                                            ...
                                            <input type="hidden" id="regprogramModal{{ $key }}" value="{{ $item->program }}">
                                            <input type="hidden" id="regcourseTimeModal{{ $key }}" value="{{ $item->course_time }}">
                                            <input type="hidden" id="regday1Modal{{ $key }}" value="{{ $item->day1 }}">
                                            <input type="hidden" id="regday2Modal{{ $key }}" value="{{ $item->day2 }}">
                                            <input type="hidden" id="regteacherModal{{ $key }}" value="{{ $item->teacher_name }}">
                                            <input type="hidden" id="regteacherOldModal{{ $key }}" value="{{ $item->id_teacher }}">
                                            <input type="hidden" id="regclassModal{{ $key }}" value="{{ $item->priceid }}">
                                            <input type="hidden" id="regdayOneModal{{ $key }}" value="{{ $item->day_one }}">
                                            <input type="hidden" id="regdayTwoModal{{ $key }}" value="{{ $item->day_two }}">
                                            <input type="hidden" id="regidteacherModal{{ $key }}" value="{{ $item->id_teacher }}">
                                        </span>
            ...
            <div class="modal fade" id="editStarModal" tabindex="-1" role="dialog"
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <form action="#" method="POST" id="formStarSelection">
                            @csrf
                            ...
                            <div class="modal-body text-center">
                                ...
                                <div class="d-flex justify-content-around align-items-center">
                                    ...
                                    <button type="button" class="btn btn-outline-success btn-huge star-select-btn" data-star-value="star_1" style="width: 45%; height: 120px; font-size: 24px;">
                                        <i class="fas fa-star fa-3x d-block mb-1"></i>
                                        <span class="d-block" style="font-size: 16px;">Star 1</span>
                                    </button>

                                    <button type="button" class="btn btn-outline-primary btn-huge star-select-btn" data-star-value="star_2" style="width: 45%; height: 120px; font-size: 24px;">
...
<script>
    function updateModalReg(id) {
        ...
        var url = "{{ url('attendance/update-class') }}";
        $('#updateClassModal').attr('action', url);
    ...
    $(document).ready(function() {
      // --- LOGIKA PEMILIHAN STAR (Sudah ada, hanya dipertahankan) ---
        ...
        $('#editStarModal').on('show.bs.modal', function(event) {
            ...
            var modal = $(this);
            modal.find('#modal_priceid').val(priceid);
            modal.find('#modal_day1').val(day1);
            modal.find('#modal_day2').val(day2);
            modal.find('#modal_course_time').val(courseTime);
            modal.find('#modal_id_teacher').val(idTeacher);
