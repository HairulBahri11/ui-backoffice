...
class BirthdayServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        $getBirthdayNotifications = function () {
            ...
            // 1. Ambil data siswa yang sudah dapat poin tahun ini
            $studentsWithPointsThisYear = DB::table('attendance_detail_points')
                ->join('attendance_details', 'attendance_details.id', '=', 'attendance_detail_points.attendance_detail_id')
                ->where('attendance_detail_points.point_category_id', 7)
                ->whereYear('attendance_detail_points.created_at', $currentYear)
            ...
            foreach ($studentListActive as $item) {
                ...
                // --- LOGIKA BARU: CEK TANGGAL INACTIVE ---
                if (isset($lastInactiveDates[$item->id])) {
                    $lastInactive = Carbon::parse($lastInactiveDates[$item->id]);

                    // Set tahun ulang tahun ke tahun ini untuk perbandingan validitas di tahun berjalan
                    $birthdayThisYear = clone $birthdayDate;
                    $birthdayThisYear->year((int)$currentYear);

                    // Jika tanggal nonaktif terakhir terjadi SEBELUM ulang tahun di tahun ini, tiadakan/skip
                    if ($lastInactive->lessThan($birthdayThisYear)) {
                        continue;
                    }
                }
                // ----------------------------------------
                ...
                // Catatan: Variabel nama $birthdayPlus7DaysDate dipertahankan dari kode asli Anda, 
                // namun metodenya menambahkan 30 hari sesuai implementasi awal Anda.
                $birthdayPlus7DaysDate = clone $birthdayDate;
                $birthdayPlus7DaysDate->addDays(30);
                $birthdayPlus7DaysMonthDay = $birthdayPlus7DaysDate->format('m-d');
